...
class ArticleSearch {
   ...
   */
  static async querySearch(type, searchWord, searchlimit, pageoffset) {
   ...
   * @memberof ArticleSearch
   */
  static async filterSearch(type, searchWord, filterObject, searchlimit, pageoffset) {
    const limit = searchlimit || 20;
    const offset = pageoffset || 0;
    ...
    const articles = await Article.findAndCountAll({
      ...
      /* include: [
        {
          ...
          attributes: ['firstname', 'lastname', 'username', 'image'], duplicating: false
        },
        {
          model: Categories, attributes: ['name'], through: { attributes: [] }, duplicating: false
        },
      ...
      where: { [Op.and]: { type, [Op.or]: queryFields } }
    });
    return articles;
  }
}
